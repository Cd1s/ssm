...
Review BC-1 through BC-10 against the table below; review architecture D01
through D14 against the decision register; verify release blockers above,
rollback assumptions, the provenance runbook, and every public help/README,
agent, schema, security, and release-note surface. The implementation evidence
is indexed in `docs/plans/issue-20-*` through `issue-29-*`, with the standalone
BC-7 transfer matrix and verification manifest.
