...
func uploadFileResumable(c config.Connection, v *config.Vault, localPath, remotePath string, opts UploadOptions) (TransferResult, error) {
	...
	if opts.ResumeVersion != resumeProtocolV1 {
		return result, transferError(machinecontract.ResumeVersionUnsupported, 0, fmt.Errorf("unsupported resume version %q", opts.ResumeVersion))
	}
	...
	if err != nil || !info.Mode().IsRegular() {
		...
		return result, transferError(machinecontract.ResumeRegularFileRequired, 0, err)
	}
	h := sha256.New()
	if _, err := io.Copy(h, f); err != nil {
		return result, transferError(machinecontract.ResumeSourceReadFailed, 0, err)
	}
	...
	if err != nil {
		return result, transferError(machinecontract.ResumeSourceChanged, 0, err)
	}
	if prefixDigest != state.Digest {
		result.Stage = "resume_validate"
		result.Integrity = "mismatch"
		return result, transferError(machinecontract.ResumePartialMismatch, 0, errors.New("remote partial digest does not match local prefix"))
	}
	...
	if err != nil {
		return result, transferError(machinecontract.ResumeStdinOpenFailed, 0, err)
	}
	...
	if err := session.Start(cmd); err != nil {
		return result, transferError(machinecontract.ResumeStartFailed, 0, err)
	}
	...
	if err := session.Wait(); err != nil {
		if timedOut.Load() {
			return result, transferError(machinecontract.ResumeTimedOut, written, err)
		}
		...
		case strings.Contains(marker, "SSM_RESUME_ERROR digest_mismatch"):
			result.Integrity = "mismatch"
			return result, transferError(machinecontract.ResumeIntegrityMismatch, written, errors.New("remote final digest mismatch"))
		case strings.Contains(marker, "SSM_RESUME_ERROR publish_failed"):
			return result, transferError(machinecontract.ResumePublishFailed, written, errors.New("atomic publish failed"))
		default:
			message := strings.TrimSpace(stderr.String())
			if message == "" {
				message = err.Error()
			}
			return result, transferError(machinecontract.ResumeRetryFailed, written, errors.New(message))
		}
	}
	remoteSize, remoteDigest, err := parseUploadReceipt(stdout.String())
	if err != nil || remoteSize != info.Size() || remoteDigest != fullDigest {
		result.Integrity = "mismatch"
		return result, transferError(machinecontract.ResumeReceiptInvalid, written, errors.New("invalid resume completion receipt"))
	}
	result.OK = true
...
func probeResumeState(client *gossh.Client, partial, metadata, cleanupPattern string, size int64, digest string) (resumeState, error) {
	...
	if err := session.Run(command); err != nil {
		...
		default:
			...
			return resumeState{}, transferError(machinecontract.ResumeProbeFailed, 0, errors.New(message))
		}
	}
	fields := strings.Fields(strings.TrimSpace(stdout.String()))
	if len(fields) != 3 || fields[0] != "SSM_RESUME" {
		return resumeState{}, transferError(machinecontract.ResumeProbeResponseInvalid, 0, errors.New("invalid remote resume state"))
	}
	offset, err := strconv.ParseInt(fields[1], 10, 64)
	if err != nil || offset < 0 || offset > size || len(fields[2]) != 64 {
		return resumeState{}, transferError(machinecontract.ResumeProbeResponseInvalid, 0, errors.New("invalid remote resume offset or digest"))
	}
