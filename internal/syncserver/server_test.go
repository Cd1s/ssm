...
func TestServerStoresPrivateFiles(t *testing.T) {
	...
	for _, path := range []string{
		...
		if err := privatepath.VerifyDirectory(path); err != nil {
			t.Fatalf("%s is not a private directory: %v", path, err)
		}
	}
	if err := privatepath.VerifyFile(filepath.Join(dir, "users.json")); err != nil {
		t.Fatalf("users file is not private: %v", err)
	}
	...
	if err := privatepath.VerifyFile(filepath.Join(dir, "vaults", entries[0].Name())); err != nil {
