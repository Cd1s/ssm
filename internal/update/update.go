...
// the one approved next-major migration target. Drafts, prereleases, malformed
// versions, skipped major versions, and versions that are not newer than the
// running binary are unsupported.
func SelectRelease(releases []Release, current string) (sameMajor, crossMajor *Release) {
	...
	for i := range releases {
		...
		} else if version.major == currentVersion.major+1 {
			if crossMajor == nil {
