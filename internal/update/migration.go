...
// ReviewMajor prepares the approved v1-to-v2 review and v1-format preflight.
// Authorization is recorded separately and never authenticates an artifact.
...
func checkCloudConfiguration() MigrationCheck {
	...
	data, err := os.ReadFile(path) //nolint:gosec // fixed cloud.json path beneath SSM's configured private directory
	if err != nil {
	...
	var value cloud.CloudConfig
	if err := json.Unmarshal(data, &value); err != nil {
		return failedCheck("cloud_configuration", "Cloud configuration is invalid or unreadable.", "Repair or deliberately remove cloud.json before migration.")
	}
	server, err := url.Parse(strings.TrimSpace(value.Server))
	if err != nil || (server.Scheme != "http" && server.Scheme != "https") || server.Host == "" || strings.TrimSpace(value.Token) == "" {
		return failedCheck("cloud_configuration", "Cloud configuration is invalid or unreadable.", "Repair or deliberately remove cloud.json before migration.")
	}
...
func checkPendingRecovery(masterPassPath string) MigrationCheck {
	...
	password, err := os.ReadFile(passwordPath) //nolint:gosec // explicit user-supplied master-pass path is the supported non-interactive interface
	if err != nil {
...
func checkDivergence() MigrationCheck {
	path := filepath.Join(config.Dir(), "sync-conflict.json")
	if _, err := os.ReadFile(path); err == nil || !os.IsNotExist(err) { //nolint:gosec // fixed conflict record beneath SSM's private directory
...
func checkReleaseAsset(release Release) MigrationCheck {
	...
	if err := releaseasset.ValidateReleaseNames(names); err != nil {
		return failedCheck("platform_asset", "The target release does not declare the exact supported release asset and provenance manifest.", "Install only after the release publishes the exact six supported assets, adjacent provenance bundles, installer, and digest data.")
	}
	wanted := assetName()
	...
	return failedCheck("platform_asset", "The target release does not support this platform.", "Use a supported linux, darwin, or windows amd64/arm64 target.")
}
