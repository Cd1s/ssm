...
func TestParsePutReliabilityOptions(t *testing.T) {
	...
	opts, err := parsePutArgs([]string{"prod", "local.bin", "/srv/remote.bin", "--sha256", "--timeout=30s", "--json"})
	if err != nil {
		t.Fatal(err)
	}
	if opts.name != "prod" || opts.localPath != "local.bin" || opts.remotePath != "/srv/remote.bin" || !opts.verifySHA256 || opts.timeout != 30*time.Second || !machineJSON {
